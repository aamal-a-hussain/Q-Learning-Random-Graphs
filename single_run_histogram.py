...
def run_single_game():
    ...
    network = create_sbm_game(0.2, 0.7)
    ...
    x, Q_values, traj = QL.run_ql(x, Q_values, N_ITER, edgeset, game, N_AGENTS, N_ACTIONS, T=T)
    traj = traj.T

    window_traj = window_traj = traj[:, -window_size:].reshape((N_AGENTS, N_ACTIONS, window_size))
    maximum_extents = (window_traj.max(axis=-1) - window_traj.min(axis=-1)).max(axis=-1)
    return [(i, e) for i, p in enumerate(network.graph['partition']) for e in maximum_extents[list(p)]]

def run_multiple_experiments(n_expt=128):
...
if __name__ == "__main__":
    ...
    maximum_extents = run_multiple_experiments()
