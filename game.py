...
class NetworkGame:
    n_agents: int
    n_actions: int
    payoff_A: jnp.ndarray | None = None
    payoff_B: jnp.ndarray | None = None
    ...
    def get_payoffs(self, x, edgeset):
        ...
        x = jnp.array(x)
        edges = jnp.array(edgeset)
        return _get_jit_payoffs(
...
class ShapleyGame(NetworkGame):
    ...
    def __init__(self, n_agents, beta=0.2):
        ...
        self.payoff_A = jnp.array([[1, 0, beta], [beta, 1, 0], [0, beta, 1]])
        self.payoff_B = jnp.array([[-beta, 1, 0], [0, -beta, 1], [1, 0, -beta]])
    ...
    def get_payoffs(self, x, edgeset):
        ...
        x = jnp.array(x)
        edges = jnp.array(edgeset)
        return _get_jit_payoffs_no_edges(
            x, edges, self.payoff_A, self.payoff_B, self.n_agents, self.n_actions
        )
...
class SatoGame(NetworkGame):
    ...
    def __init__(self, n_agents, ex=0.1, ey=0.05):
        assert n_agents > 2
        self.n_agents = n_agents
        self.payoff_A = jnp.array([[ex, -1, 1], [1, ex, -1], [-1, 1, ex]])
        self.payoff_B = jnp.array([[ey, -1, 1], [1, ey, -1], [-1, 1, ey]])
...
def _get_payoffs_general(x, edges, payoff_A, payoff_B, n_agents, n_actions):
    ...
    contrib_0 = jnp.einsum("ije, je -> ie", payoff_A, x_nbr_0.T)
    contrib_1 = jnp.einsum("ije, je -> ie", payoff_B, x_nbr_1.T)
...
def _get_payoffs_no_edges(x, edges, payoff_A, payoff_B, n_agents, n_actions):
    ...
    contrib_0 = jnp.einsum("ij, je -> ie", payoff_A, x_nbr_0.T)
    contrib_1 = jnp.einsum("ij, je -> ie", payoff_B, x_nbr_1.T)
